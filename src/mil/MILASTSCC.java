...
public class MILASTSCC {

  /** Records the list of X\s in this binding group. */
  private MILASTs bindings = null;

  /** Return the list of X\s in this scc. */
  public MILASTs getBindings() {
    return bindings;
  }

  /** Add an X to this scc. */
  public void add(MILAST binding) {
    bindings = new MILASTs(binding, bindings);
  }
   ...
   */
  private MILASTSCCs dependsOn = null;

  public MILASTSCCs getDependsOn() {
    return dependsOn;
  }
   ...
   */
  static void addDependency(MILASTSCC from, MILASTSCC to) {
    if (from != to && !find(to, from.dependsOn)) {
      from.dependsOn = new MILASTSCCs(to, from.dependsOn);
    }
  }

  /** Search for a specific binding scc within a given list. */
  static boolean find(MILASTSCC scc, MILASTSCCs sccs) {
