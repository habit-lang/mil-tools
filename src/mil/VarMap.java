...
abstract class VarMap {
  ...
  /**
   * Stores the mapping from MIL temporaries to llvm locals for this CFG. We assume that the MIL
   * temporaries used here have already been translated, if necessary, by the substitution to
   * identify distinct Block parameters that use the same temporary.
   */
   ...
   */
  llvm.Local lookup(TypeMap tm, Temp t) {
    llvm.Local lhs = tempMap.get(t);
    if (lhs == null) {
      lhs = reg(t.lookupType(tm));
      tempMap.put(t, lhs);
    }
    return lhs;
  }
  ...
  abstract llvm.Value lookupGlobal(TypeMap tm, Top t);
}
