...
public class If extends Code {
  ...
  public Code flow(Defn d, Facts facts, TempSubst s) { // if a then ifTrue else ifFalse
    a = a.apply(s);
    Flag c = a.isFlag(); // Does the argument hold a known constant?
    if (c != null) {
      BlockCall bc = c.getVal() ? ifTrue : ifFalse;
      return new Done(bc.applyBlockCall(a.mapsTo(c, s)).rewriteBlockCall(d, facts, false));
    } else {
    ...
    ifTrue = ifTrue.applyBlockCall(a.mapsTo(Flag.True, s)).rewriteBlockCall(d, facts, false);
    ifFalse = ifFalse.applyBlockCall(a.mapsTo(Flag.False, s)).rewriteBlockCall(d, facts, false);
  ...
  void calcSources(Defn d, Temp[] params, Temps rebound) {
    ifTrue.calcSources(d, params, rebound);
    ifFalse.calcSources(d, params, rebound);
  }
