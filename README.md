...
we use in these demonstrations is in a source file called `ex.lc`
...
as in `java -jar mil-tools.jar -m ex.lc -p`.  This, however,
...
LC source files):
    ...
    $ java -jar mil-tools.jar -m ex.lc -po 
    data Dup (a::*)
      = Dup a a
...
function rewrite") as part of the `-p` setting:
    ...
    $ java -jar mil-tools.jar -m ex.lc -pco
    data Dup (a::*)
      = Dup a a
...
is shown here by using `-pcso`, which only uses one optimizer pass.)
    ...
    $ java -jar mil-tools.jar -m ex.lc -pcoso
    data Dup0
      = Dup0 (Bit 7) (Bit 7)
...
details:
    ...
    $ java -jar mil-tools.jar -m ex.lc -pcosoro
    data Dup0
      = Dup0 Word Word
...
information that is generated during specialization:
    ...
    $ java -jar mil-tools.jar -m ex.lc -pcosbo
    bitdata Dup2 /6
...
final output shown below:
    ...
    $ java -jar mil-tools.jar -m ex.lc -pcosboro
    -----------------------------------------
    -- not recursive
...
to follow.)
    ...
    $ java -jar mil-tools.jar -l ex.lc -pcosboro
    @d1 = global i32 14
