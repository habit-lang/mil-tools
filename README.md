...
suitable installation of LLVM):
    ...
    java -jar mil-tools.jar demo/fib.lc -ltmp/fib.ll --main
...
the program directly through the bytecode interpreter:
    ...
    java -jar mil-tools.jar -x demo/fib.lc --main
...
If you just want to have mil-tools load, check, and compile
this program to MIL, you can use the command line option `-p`,
as in `java -jar mil-tools.jar -m demo/ex.lc -p`.  This, however,
will produce quite a lot of output, and we can simplify the
code a little by adding the character `o` to the end of the
`-p` command line option, producing the following output
(more generally, the `-p` option takes a sequence of letters,
each of which specifies a particular pass; mil-tools will
then attempt to run exactly that sequence of passes on the
MIL code that is loaded in directly or generated by compiling
LC source files):

    $ java -jar mil-tools.jar -m demo/ex.lc -po 
    data Dup (a::*)
      = Dup a a
...
function rewrite") as part of the `-p` setting:
    ...
    $ java -jar mil-tools.jar -m demo/ex.lc -pco
    data Dup (a::*)
      = Dup a a
...
is shown here by using `-pcso`, which only uses one optimizer pass.)
    ...
    $ java -jar mil-tools.jar -m demo/ex.lc -pcoso
    data Dup0
      = Dup0 (Bit 7) (Bit 7)
...
details:
    ...
    $ java -jar mil-tools.jar -m demo/ex.lc -pcosoro
    data Dup0
      = Dup0 Word Word
...
information that is generated during specialization:
    ...
    $ java -jar mil-tools.jar -m demo/ex.lc -pcosbo
    bitdata Dup2 /6
...
final output shown below:
    ...
    $ java -jar mil-tools.jar -m demo/ex.lc -pcosboro
    -----------------------------------------
    -- not recursive
...
to follow.)
    ...
    $ java -jar mil-tools.jar -l demo/ex.lc -pcosboro
    @d1 = global i32 14
